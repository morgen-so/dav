...
suite('request.calendarQuery', function () {
  ...
  test(
    'should add specified props to report body',
    co.wrap(function* () {
      let mock = nockWrapper('http://127.0.0.1:1337').matchRequestBody(
        '/principals/admin/',
        'REPORT',
        (body) => {
  ...
  test(
    'should add specified filters to report body',
    co.wrap(function* () {
      let mock = nockWrapper('http://127.0.0.1:1337').matchRequestBody(
        '/principals/admin/',
        'REPORT',
  ...
  test(
    'should add timezone to report body',
    co.wrap(function* () {
      let mock = nockWrapper('http://127.0.0.1:1337').matchRequestBody(
        '/principals/admin/',
        'REPORT',
  ...
  test(
    'should resolve with appropriate data structure',
    co.wrap(function* () {
