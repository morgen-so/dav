...
suite('request.syncCollection', function () {
  ...
  test(
    'should add props to request body',
    co.wrap(function* () {
      ...
      let send = xhr.send(
        req,
        'http://127.0.0.1:1337/principals/admin/default/'
      );
  ...
  test(
    'should set sync details in request body',
    co.wrap(function* () {
      let mock = nockWrapper('http://127.0.0.1:1337').matchRequestBody(
        '/principals/admin/default/',
        'REPORT',
        (body) => {
          return (
            body.indexOf('<d:sync-level>1</d:sync-level>') !== -1 &&
