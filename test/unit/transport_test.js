...
suite('Basic#send', function () {
  ...
  setup(function () {
    xhr = new Basic(new Credentials({ username: 'admin', password: 'admin' }));
  ...
  test(
    'should send req',
    co.wrap(function* () {
      let nockObj = nock('http://127.0.0.1:1337').get('/').reply(200, '200 OK');
  ...
  test('should invoke onerror if error thrown', function (done) {
    nock('http://127.0.0.1:1337').get('/').reply(400, '400 Bad Request');
  ...
  test(
    'should return promise that resolves with xhr',
    co.wrap(function* () {
      nock('http://127.0.0.1:1337').get('/').reply(200, '200 OK');
...
suite.skip('OAuth2#send', function () {
  ...
  test(
    'should refresh access token if expired',
    co.wrap(function* () {
      ...
      credentials.refreshToken =
        '1/oPHTPFgECWFPrs7KgHdis24u6Xl4E4EnRrkkiwLfzdk';
  ...
  test(
    'should use provided access token if not expired',
    co.wrap(function* () {
