...
suite('XMLHttpRequestWrapper#send', function () {
  ...
  test(
    'should reject with statusText if status >=400',
    co.wrap(function* () {
      nock('http://127.0.0.1:1337')
        .get('/')
  ...
  test(
    'should reject with timeout error on timeout',
    co.wrap(function* () {
      nock('http://127.0.0.1:1337').get('/').delay(2).reply(200, '200 OK');
  ...
  test(
    'should resolve with responseText if everything ok',
    co.wrap(function* () {
