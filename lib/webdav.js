...
let debug = require('./debug').default('dav:webdav');
...
export function syncCollection(collection, options) {
  ...
  if ('syncMethod' in options) {
    ...
    collection.reports &&
    collection.reports.indexOf('sync-collection') !== -1
  ) {
    syncMethod = 'webdav';
  } else {
    syncMethod = 'basic';
...
export let isCollectionDirty = co.wrap(function* (collection, options) {
  if (!collection.ctag) {
    debug('Missing ctag.');
    return true; // of course, if ctag is missing the collection needs sync
  }
  ...
  let responses = yield options.xhr.send(req, collection.url, {
    sandbox: options.sandbox,
  });
