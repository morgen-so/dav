...
export let listCalendarObjects = co.wrap(function* (calendar, options) {
	...
	options.hrefs = results.filter(res => res.href && res.href.length).map(res => res.href);
...
export let multigetSingleCalendarObject = co.wrap(function* (calendar, options) {
...
let webdavSync = co.wrap(function* (calendar, options) {
	...
	let deletedHrefs = result.responses.filter(res => res.href && res.href.length && res.status && res.status.length && res.status.indexOf('404') > -1).map(res => res.href);
	let newUpdatedHrefs = result.responses.filter(res => !res.status || res.status.indexOf('404') === -1).map(res => res.href);
	...
	if (calendar.url.indexOf("icloud.com") > -1) {
		newUpdatedHrefs = newUpdatedHrefs.filter(href => href.indexOf(".ics") > -1)
	}
	...
	results.forEach(function (response) {
		...
		let calendarObject = calendar.objects.filter(function (object) {
			return fuzzyUrlEquals(ensureDecodedPath(object.url), response.href);
		})[0];
		...
		if (!calendarObject) {
			...
			calendar.objects.push(
				new CalendarObject({
					data: response,
					calendar: calendar,
					url: url.resolve(calendar.account.principalUrl, response.href),
	...
	calendar.objects = calendar.objects.filter(function (object) {
		return !deletedHrefs.some(function (del) {
			return fuzzyUrlEquals(ensureDecodedPath(object.url), del);
		})
	});
