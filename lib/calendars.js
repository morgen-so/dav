...
export let multigetSingleCalendarObject = co.wrap(function *(calendar, options) {
	let events = multigetCalendarObjects(calendar, options);
	return events.filter(event => {
		// Find the response that corresponds to the parameter collection.
		return fuzzyUrlEquals(options.href, event.href);
	})[0];
});
...
let webdavSync = co.wrap(function *(calendar, options) {
  ...
  let deletedHrefs = result.responses.filter(res => res.status && res.status.indexOf('404') > -1).map(res => res.href);
