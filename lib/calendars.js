...
let webdavSync = co.wrap(function* (calendar, options) {
  ...
  newUpdatedHrefs = newUpdatedHrefs.map((href) => ensureEncodedPath(href));
  ...
  let results = [];
  try {
    results = yield options.xhr.send(req, calendar.url, {
      sandbox: options.sandbox,
    });
  } catch (err) {
    if (err.code && err.code === 404) {
      ...
      results = yield listCalendarObjectsInSeries_(
        newUpdatedHrefs,
        options,
        calendar
      );
