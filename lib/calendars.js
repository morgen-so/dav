...
export let listCalendarObjects = co.wrap(function *(calendar, options) {
	...
	let objectEtags = yield listCalendarObjectsEtags({
	...
	let req = request.calendarMultiget({
		...
		hrefs: objectEtags.map(res => res.props.getetag)
	});
...
let webdavSync = co.wrap(function *(calendar, options) {
  ...
  // Detect deleted objects as objects with 'null' calendarData.
	let newOb
  let deletedHrefs = result.result.responses.filter(res => res.status.indexOf('404') > -1);
  let newUpdatedHrefs = result.result.responses.filter(res => res.status.indexOf('404') === -1).map(res => res.href);

  result = yield request.calendarMultiget({
		props: [
			{ name: 'getetag', namespace: ns.DAV },
			{ name: 'calendar-data', namespace: ns.CALDAV }
		],
