...
let debug = require('./debug').default('dav:contacts');
...
export let listAddressBooks = co.wrap(function* (account, options) {
  ...
  yield addressBooks.map(
    co.wrap(function* (addressBook) {
      addressBook.reports = yield webdav.supportedReportSet(
        addressBook,
        options
      );
    })
...
export let listVCards = co.wrap(function* (addressBook, options) {
  ...
  return responses.map((res) => {
    ...
    return new VCard({
      ...
      url: url.resolve(addressBook.account.principalUrl, res.href),
...
export let syncCarddavAccount = co.wrap(function* (account, options = {}) {
  ...
  yield account.addressBooks.map(
    co.wrap(function* (addressBook, index) {
      try {
