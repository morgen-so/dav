...
import { getPreferredEmail } from './parser';
 ...
 */
let preferredEmail = co.wrap(function* (account, options) {
  debug(`Fetch address set from principal url ${account.principalUrl}.`);
  let prop = { name: 'calendar-user-address-set', namespace: ns.CALDAV };
  ...
  let xhrObj = yield options.xhr.send(req, account.principalUrl, {
    sandbox: options.sandbox,
  });

  return getPreferredEmail(xhrObj._responseText)?.replace('mailto:', '');
});
 ...
 */
let calendarUserProxies = co.wrap(function* (account, options) {
  debug(
    `Fetch calendar user proxies from principal URL ${account.principalUrl}.`
  );
  let req = request.propfind({
    props: [
      { name: 'calendar-proxy-read-for', namespace: ns.CALENDAR_SERVER },
      { name: 'calendar-proxy-write-for', namespace: ns.CALENDAR_SERVER },
    ],
    depth: 0,
    mergeResponses: true,
  });
  let res = yield options.xhr.send(req, account.principalUrl, {
    sandbox: options.sandbox,
  });

  let container = res.props;
  debug(`Received calendar user proxies: ${JSON.stringify(container)}`);
...
exports.createAccount = co.wrap(function* (options) {
  ...
  if (options.accountType === 'caldav') {
    ...
    account.preferredEmail = yield preferredEmail(account, options);
  }
  ...
  if (options.accountType === 'caldav' && options.loadUserProxyCalendars) {
    const userProxies = yield calendarUserProxies(account, options);
    debug(`Parsed calendar user proxies: ${JSON.stringify(userProxies)}`);
    proxyCalendars = yield userProxies.map(
      co.wrap(function* (principal) {
        let proxyAccount = new Account({
          server: options.server,
          credentials: options.xhr.credentials,
        });
        proxyAccount.principalUrl = url.resolve(
          account.principalUrl,
          principal
        );
        debug(`User proxy principal URL: ${proxyAccount.principalUrl}`);
        proxyAccount.homeUrl = yield homeUrl(proxyAccount, options);
        proxyAccount.addresses = yield addressSet(proxyAccount, options);
        proxyAccount.preferredEmail = yield preferredEmail(
          proxyAccount,
          options
        );
        return yield listCalendars(proxyAccount, options);
      })
    );
    proxyCalendars = proxyCalendars.flat().map((cal) => {
      cal.isDelegate = true;
      return cal;
    });
  }
  ...
  if (options.accountType === 'caldav' && options.loadUserProxyCalendars) {
    account[key].push(...proxyCalendars);
  }
